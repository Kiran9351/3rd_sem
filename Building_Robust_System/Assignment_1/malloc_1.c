...
{
	int free_index = 0, header_index = 0;
	void *return_ptr = NULL;

	header_index = search_free_index(iSize);
	...
	free_index = *(int*)(arr + header_index);
	
//	*(int*)(arr + header_index) = free_index + iSize;

	return_ptr = arr + (free_index);
	
	manage_header(header_index,iSize);
